//
//  FavouriteDrawerViewController.m
//  D2brain
//
...
#import "FavouriteDrawerViewController.h"
#import "FavouriteDrawerTableViewCell.h"
#import "MBProgressHUD.h"
#import "Reachability.h"
#import "UIView+Toast.h"

@interface FavouriteDrawerViewController ()
{
    FavouriteDrawerTableViewCell *cell;
...
@implementation FavouriteDrawerViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([[[componentArr objectAtIndex:indexPath.row] objectAtIndex:3]isEqualToString:@"switch"]) {
        ...
        cell=(FavouriteDrawerTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"CellSwitch"];
        ...
        cell=(FavouriteDrawerTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"CellDimmer"];
...
-(void)btnChangeDimmerSwitchState:(UIButton*)sender {
    ...
    if ([self checkInternetConnectivity:[[componentArr objectAtIndex:sender.tag] objectAtIndex:10]] ==YES) {
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            NSLog(@"--------%@",callURL);
            callURL=[NSString stringWithFormat:@"http://%@/cdmcr.cgi?DM=%@%@%@",@"192.168.1.220",switchNo,switchState,[[componentStatus objectAtIndex:sender.tag] substringWithRange:NSMakeRange(2, 2)]];
...
-(void)fetchAllFavouriteComponent{
    ...
    NSLog(@"Component Array--->> %@",componentArr);
}
...
-(void) fetchMachineFromDatabase {
    ...
    NSLog(@"MAchine Array--->> %@",machineArray);
