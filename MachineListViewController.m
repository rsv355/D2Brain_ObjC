...
@interface MachineListViewController ()<EditMachineViewControllerDelegate,MachineDialogueViewControllerDelegate>
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([[[machineArr objectAtIndex:indexPath.row]objectAtIndex:3] length]==0) {
        [cell.switchWorldWideList setHidden:YES];
        [cell.viewWorldWideHeightConstList setConstant:0.0f];
        [cell.viewLineHeightconstList setConstant:0.0f];
    }
    else{
        [cell.switchWorldWideList setHidden:NO];
        [cell.viewWorldWideHeightConstList setConstant:49.0f];
        [cell.viewLineHeightconstList setConstant:1.0f];
        
    }
    cell.switchMachineList.transform = CGAffineTransformMakeScale(0.75, 0.75);
    cell.switchWorldWideList.transform = CGAffineTransformMakeScale(0.75, 0.75);
    
    if ([[[machineArr objectAtIndex:indexPath.row]objectAtIndex:4] integerValue]==0) {
        cell.switchMachineList.on=NO;
    }
    else{
        cell.switchMachineList.on=YES;
    }
    if ([[[machineArr objectAtIndex:indexPath.row]objectAtIndex:10] integerValue]==0) {
        cell.switchWorldWideList.on=NO;
    }
    else{
        cell.switchWorldWideList.on=YES;
        
    }
    [cell.lblMachineNameList setText:[[machineArr objectAtIndex:indexPath.row]objectAtIndex:1]];
    [cell.lblMachinIPAddressList setText:[[machineArr objectAtIndex:indexPath.row]objectAtIndex:2]];
    [cell.lblMachinSerialNoList setText:[[machineArr objectAtIndex:indexPath.row]objectAtIndex:6]];
    [cell.lblWorldWideIPList setText:[[machineArr objectAtIndex:indexPath.row]objectAtIndex:3]];
    [cell.btnDeleteList setTag:indexPath.row];
    [cell.btnEditList setTag:indexPath.row];
    [cell.switchMachineList setTag:indexPath.row];
    [cell.switchWorldWideList setTag:indexPath.row];
    
    [cell.btnDeleteList addTarget:self
                       action:@selector(btnDeleteMachine:) forControlEvents:UIControlEventTouchDown];
    [cell.btnEditList addTarget:self
                     action:@selector(btnEditMachine:) forControlEvents:UIControlEventTouchDown];
    [cell.switchMachineList addTarget:self
                           action:@selector(btnSwitchMachine:) forControlEvents:UIControlEventValueChanged];
    
    [cell.switchWorldWideList addTarget:self
                             action:@selector(btnSwitchWorldwide:) forControlEvents:UIControlEventValueChanged];
    if ([machineArr count]==1) {
        cell.btnDeleteWidthConstList.constant=0.0f;
    }
    else {
        cell.btnDeleteWidthConstList.constant=26.0f;
    }
    return cell;
}
